...
{
    [TestMethod]
    public async Task Watch_callback_nao_deve_ser_disparado_sem_chamada_de_NotifyChanged()
    {
        DebounceChangeNotifier<string, int> notifyChanges = new(50);
        CancellationTokenSource cts = new(200);

        int changesCount = 0;
        await notifyChanges.Watch(async changes =>
        {
    ...
    [TestMethod]
    public async Task Changes_em_Watch_callback_deve_corresponder_as_chamadas_de_NotifyChanged_com_keys_diferentes()
    {
        DebounceChangeNotifier<string, int> notifyChanges = new(50);
    ...
    [TestMethod]
    public async Task Chamadas_sucessivas_de_NotifyChanged_com_mesma_key_deve_gerar_apenas_uma_change_com_ultimo_valor_em_Watch_callback()
    {
        DebounceChangeNotifier<string, int> notifyChanges = new(50);
        CancellationTokenSource cts = new(60_000); // Ao debugar, levar em conta o timeout
